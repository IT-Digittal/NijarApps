...
Plataforma DTI (Destino Turístico Inteligente) Smart City for the Ayuntamiento de Níjar. A FastAPI backend serving a tourism platform with IoT ingestion, social listening, multilingual chatbot, CMS, and dashboards. The codebase is primarily in Spanish (comments, variable names, docs).
...
- **`api/v1/`** — 8 routers mounted under `/api/v1`: auth, tourism, iot, social, cms, chatbot, dashboards, health
...
- **Static frontends**: Dashboard and totem UI are plain HTML/JS in `frontend/`, served by FastAPI's StaticFiles — no build step
