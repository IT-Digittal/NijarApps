...
class TestRect:
    ...
    def test_centrado(self):
        ...
        coords = [tuple(float(x) for x in p.split()) for p in
                  wkt.replace("POLYGON((", "").replace("))", "").split(",")]
...
class TestEsquemasGeoJSON:
    ...
    def test_feature_collection(self):
        capa = CapaGeograficaOut(
            ...
            codigo="x", nombre="X", grupo="catastro", tipo_geometria="poligono",
            color="#fff", color_borde="#000", opacidad=0.3, orden=0, activa=True,
        )
